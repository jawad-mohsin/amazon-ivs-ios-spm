...
typedef NS_ENUM(NSInteger, IVSBroadcastError) {
    ...
    /// The stage video maximum bitrate set on the `IVSLocalStageStreamVideoConfiguration` must be between 50k and 2,500k. This error is thrown
    ...
    /// The stage video minimum bitrate set on the `IVSLocalStageStreamVideoConfiguration` must be between 50k and 2,500k. This error is thrown
    ...
    /// The number of the stage video layers set on the `IVSLocalStageStreamVideoConfiguration` must be 3 or less. This error is thrown
    /// when the number of the stage video layers is set to a value outside this range.
    IVSBroadcastErrorConfigurationMaxLayersExceeded = 10250,

    /// Two layers set on the `IVSLocalStageStreamVideoConfiguration` must have different configuration. This error is thrown
    /// when the two layers with the same configuration is set.
    IVSBroadcastErrorConfigurationDuplicateLayersConfigured = 10251,
    
    /// The layer maximum bitrate set on the `IVSLocalStageStreamVideoConfiguration` must be between 50k and 2,500k. This error is thrown
    /// when the layer maximum bitrate is set to a value outside this range.
    IVSBroadcastErrorConfigurationInvalidLayerMaxBitrate = 10252,

    /// The layer minimum bitrate set on the `IVSLocalStageStreamVideoConfiguration` must be between 50k and 2,500k. This error is thrown
    /// when the layer minimum bitrate is set to a value outside this range.
    IVSBroadcastErrorConfigurationInvalidLayerMinBitrate = 10253,

    /// The layer target framerate set on the `IVSLocalStageStreamVideoConfiguration` must be between 10 and 30. This error is thrown
    /// when the layer target framerate is set to a value outside this range.
    IVSBroadcastErrorConfigurationInvalidLayerTargetFramerate = 10254,

    /// The layer size set on the `IVSLocalStageStreamVideoConfiguration` must have a width and height of greater than 160, less than 720, and
    /// the total number of pixels must be less than 921,600.
    /// For example, the smallest possible size is 160x160, and the biggest possible size is either 720x1280 or 1280x720..
    /// This error is thrown when the video size does not meet the specified criteria.
    IVSBroadcastErrorConfigurationInvalidLayerSize = 10255,
